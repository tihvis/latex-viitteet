...
class UserService():
    ...
    def create_new_user(self, username: str, password: str):
        if self._user_repository.is_username_taken(username):
            return False
        new_user = User(self._crypto_service.create_user_uuid(), username,
    ...
    def check_user_credentials(self, username, password, user: User):
        if user is not None and user.username == username and \
            self._crypto_service.check_password(password, user.password_hash):
            return True
        return False
